...
- Reuses the shared Motor client + collections from db.py (single connection pool)
...
async def main() -> None:
    ...
    try:
        result = await reels_collection.bulk_write(reel_ops, ordered=False)
        logger.info(
    ...
    try:
        result = await interactions_collection.bulk_write(interaction_ops, ordered=False)
        logger.info(
    ...
    try:
        result = await reel_embeddings_collection.bulk_write(embedding_ops, ordered=False)
        logger.info(
