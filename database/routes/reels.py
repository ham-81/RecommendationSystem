...
- Native async Motor calls throughout the request path.
...
async def upload_reel(
    ...
    try:
        insert_result = await reels_collection.insert_one(reel_doc)
...
async def delete_reel(public_id: str) -> dict:
    try:
        doc = await reels_collection.find_one({"public_id": public_id})
    except PyMongoError as exc:
    ...
    try:
        await reels_collection.delete_one({"_id": doc["_id"]})
    except PyMongoError as exc:
...
async def list_reels(limit: int = 50) -> dict:
    ...
    try:
        cursor = reels_collection.find({}, REEL_PROJECTION).sort("reel_id", 1).limit(limit)
...
async def get_feed(user_id: int, limit: int = 20) -> dict:
    ...
    try:
        ...
        rec = await reels_collection.database["recommendations"].find_one(
            {"user_id": user_id},
            {"_id": 0, "reel_ids": 1},
        )
    except PyMongoError as exc:
    ...
    if rec:
        ...
        try:
            ...
            reels = await cursor.to_list(length=len(ordered_reel_ids))
        except PyMongoError as exc:
            logger.exception("MongoDB query failed")
            raise HTTPException(status_code=500, detail=f"MongoDB query failed: {exc}") from exc
    ...
    try:
        cursor = reels_collection.find({}, REEL_PROJECTION).sort("like_count", -1).limit(limit)
